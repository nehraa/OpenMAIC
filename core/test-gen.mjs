...
async function runTest() {
  ...
  if (await phoneInput.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    await phoneInput.fill('+15559000002');
    await page.locator('button[type="submit"]').click();
    await page.waitForTimeout(1000);
    
    // Request OTP
    await page.locator('button:has-text("Request")').click();
  ...
  if (await generateBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    if (await promptInput.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const generateBtn2 = page.locator('button:has-text("Generate Slides")').last();
      ...
      console.log('Prompt input not visible, checking page...');
      const result = await page.evaluate(() => document.body.innerText);
      console.log(result.substring(0, 500));
    }
  } else {
