...
export async function POST(request: NextRequest) {
  try {
    ...
    // If no class_id provided, get the student's first enrolled class
    ...
    if (!targetClassId) {
      ...
      const classRow = classResult.rows[0] as { class_id: string } | undefined;
    ...
    return NextResponse.json({ id: questionId, status: 'pending' }, { status: 201 });
  } catch (error) {
