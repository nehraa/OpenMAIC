...
export async function getClassProgress(classId: string, filters?: ProgressFilters): Promise<ClassProgressResult | null> {
  ...
  const versionIds = assignments
    .map((a) => a.slide_asset_version_id)
    .filter((id): id is string => id !== null);

  if (versionIds.length > 0) {
    const placeholders = versionIds.map((_, i) => `$${i + 1}`).join(', ');
    const versionsResult = await db.query(`
      SELECT id, payload_json FROM content_asset_versions WHERE id IN (${placeholders})
    `, versionIds);

    const versionPayloadMap = new Map<string, string>();
    versionsResult.rows.forEach((row: any) => {
      versionPayloadMap.set(row.id, row.payload_json);
    });
    ...
    for (const assignment of assignments) {
      ...
      const payloadJson = versionPayloadMap.get(assignment.slide_asset_version_id);
