...
export async function generateQuizFromSlides(slideAssetVersionId: string): Promise<GeneratedQuestion[]> {
  ...
  try {
    if (!versionRow.payload_json || !versionRow.payload_json.trim()) {
      throw new Error('Slide content is empty - please generate slides first');
    }
...
export async function updateQuiz(id: string, data: UpdateQuizData): Promise<Quiz | null> {
  ...
  const quizResult = await db.query('SELECT id, tenant_id, owner_teacher_id, type, title, subject_tag, source_kind, source_ref, created_at, updated_at FROM content_assets WHERE id = $1 AND type = $2', [id, 'quiz']);
  ...
  const versionResult = await db.query(`
    SELECT id, tenant_id, asset_id, version_number, payload_json, status, created_at FROM content_asset_versions WHERE asset_id = $1 ORDER BY version_number DESC LIMIT 1
  `, [id]);
  ...
  const result = await db.query('SELECT id, tenant_id, owner_teacher_id, type, title, subject_tag, source_kind, source_ref, created_at, updated_at FROM content_assets WHERE id = $1', [id]);
...
export async function getQuizWithVersion(id: string): Promise<{ quiz: Quiz; version: QuizVersion; payload: QuizPayload } | null> {
  ...
  if (!version.payload_json || !version.payload_json.trim()) {
    throw new Error('Quiz content is empty');
  }
...
export async function addQuestion(quizId: string, questionData: AddQuestionData): Promise<QuizQuestion> {
  ...
  const quizResult = await db.query('SELECT id, tenant_id, owner_teacher_id, type, title, subject_tag, source_kind, source_ref, created_at, updated_at FROM content_assets WHERE id = $1 AND type = $2', [quizId, 'quiz']);
  const quiz = quizResult.rows[0] as Quiz | undefined;
  if (!quiz) {
  ...
  if (!currentVersion.payload_json || !currentVersion.payload_json.trim()) {
    throw new Error('Quiz content is empty');
  }
...
export async function updateQuestion(quizId: string, questionId: string, data: UpdateQuestionData): Promise<QuizQuestion | null> {
  ...
  const versionResult = await db.query(`
    SELECT id, tenant_id, asset_id, version_number, payload_json, status, created_at FROM content_asset_versions WHERE asset_id = $1 AND status = 'draft' ORDER BY version_number DESC LIMIT 1
  `, [quizId]);
  ...
  if (!targetVersion.payload_json || !targetVersion.payload_json.trim()) {
    throw new Error('Quiz content is empty');
  }
...
export async function getQuizById(id: string): Promise<Quiz | null> {
  const db = getDb();
  const result = await db.query('SELECT id, tenant_id, owner_teacher_id, type, title, subject_tag, source_kind, source_ref, created_at, updated_at FROM content_assets WHERE id = $1 AND type = $2', [id, 'quiz']);
