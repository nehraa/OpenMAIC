...
export async function getLibraryAssets(teacherId: string, filters?: LibraryFilters): Promise<{ assets: LibraryAsset[]; total: number }> {
  ...
  const conditions: string[] = ['ca.owner_teacher_id = $1'];
  const values: (string | number)[] = [teacherId];
  let paramIndex = 2;

  if (filters?.type) {
    conditions.push(`ca.type = $${paramIndex++}`);
    values.push(filters.type);
  }

  if (filters?.subject) {
    conditions.push(`ca.subject_tag = $${paramIndex++}`);
    values.push(filters.subject);
  }

  if (filters?.search) {
    conditions.push(`ca.title LIKE $${paramIndex++}`);
    values.push(`%${filters.search}%`);
  }

  // Class filter: filter by assets that have been used in assignments for this class
  if (filters?.classId) {
    conditions.push(` EXISTS (
      SELECT 1 FROM assignments a
      WHERE a.teacher_id = $1
      AND (
        (ca.type = 'slide_deck' AND a.slide_asset_version_id IN (
          SELECT id FROM content_asset_versions WHERE asset_id = ca.id
        ))
        OR
        (ca.type = 'quiz' AND a.quiz_asset_version_id IN (
          SELECT id FROM content_asset_versions WHERE asset_id = ca.id
        ))
      )
      AND a.class_id = $${paramIndex++}
    )`);
  ...
  const countQuery = `SELECT COUNT(*) as count FROM content_assets ca WHERE ${conditions.join(' AND ')}`;
...
export async function getAssetWithVersions(assetId: string): Promise<AssetWithVersions | null> {
  ...
  const assetResult = await db.query('SELECT id, tenant_id, owner_teacher_id, type, title, subject_tag, source_kind, source_ref, created_at, updated_at FROM content_assets WHERE id = $1', [assetId]);
  ...
  const versionsResult = await db.query(`
    SELECT id, tenant_id, asset_id, version_number, payload_json, status, created_at FROM content_asset_versions
    WHERE asset_id = $1
...
export async function reuseAsset(teacherId: string, data: ReuseAssetData): Promise<import('@shared/types/assignment').Assignment> {
  ...
  const assetResult = await db.query('SELECT id, tenant_id, owner_teacher_id, type, title, subject_tag, source_kind, source_ref, created_at, updated_at FROM content_assets WHERE id = $1 AND owner_teacher_id = $2', [data.assetId, teacherId]);
  const asset = assetResult.rows[0] as ContentAsset | undefined;
  if (!asset) {
...
export async function tagAsset(teacherId: string, data: TagAssetData): Promise<ContentAsset | null> {
  ...
  const result = await db.query('SELECT id, tenant_id, owner_teacher_id, type, title, subject_tag, source_kind, source_ref, created_at, updated_at FROM content_assets WHERE id = $1', [data.assetId]);
...
export async function updateAssetTitle(teacherId: string, assetId: string, title: string): Promise<ContentAsset | null> {
  ...
  const assetResult = await db.query('SELECT id, tenant_id, owner_teacher_id, type, title, subject_tag, source_kind, source_ref, created_at, updated_at FROM content_assets WHERE id = $1 AND owner_teacher_id = $2', [assetId, teacherId]);
  const asset = assetResult.rows[0] as ContentAsset | undefined;
  if (!asset) {
    return null;
  }
  ...
  const result = await db.query('SELECT id, tenant_id, owner_teacher_id, type, title, subject_tag, source_kind, source_ref, created_at, updated_at FROM content_assets WHERE id = $1', [assetId]);
  return result.rows[0] as ContentAsset;
}
