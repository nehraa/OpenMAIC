...
export async function createScheduleJob(assignmentId: string, releaseAt: string): Promise<SchedulerJob> {
  ...
  const existingResult = await db.query(`
    SELECT id, tenant_id, target_type, target_id, run_at, status, retry_count, last_error, created_at, updated_at FROM scheduler_jobs
  ...
  if (existingJob) {
    ...
    const result = await db.query('SELECT id, tenant_id, target_type, target_id, run_at, status, retry_count, last_error, created_at, updated_at FROM scheduler_jobs WHERE id = $1', [existingJob.id]);
...
async function handleJobFailure(db: ReturnType<typeof getDb>, jobId: string, errorMessage: string): Promise<void> {
  const jobResult = await db.query('SELECT id, tenant_id, target_type, target_id, run_at, status, retry_count, last_error, created_at, updated_at FROM scheduler_jobs WHERE id = $1', [jobId]);
  const job = jobResult.rows[0] as SchedulerJob;
