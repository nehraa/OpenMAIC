...
export async function updateSessionState(sessionId: string, state: Partial<SessionState>): Promise<LiveSession | null> {
  ...
  const result = await db.query('SELECT id, tenant_id, assignment_id, teacher_id, state_snapshot_json, status, started_at, ended_at, created_at, updated_at FROM live_sessions WHERE id = $1', [sessionId]);
  return result.rows[0] as LiveSession;
}
...
export async function getSessionWithParticipants(sessionId: string): Promise<LiveSessionWithParticipants | null> {
  ...
  const sessionResult = await db.query('SELECT id, tenant_id, assignment_id, teacher_id, state_snapshot_json, status, started_at, ended_at, created_at, updated_at FROM live_sessions WHERE id = $1', [sessionId]);
...
export async function joinSession(sessionId: string, userId: string): Promise<LiveSessionParticipant | null> {
  ...
  const existingResult = await db.query(`
    SELECT id, tenant_id, live_session_id, user_id, joined_at, left_at, completion_state FROM live_session_participants
  ...
  if (existing) {
    ...
    const result = await db.query('SELECT id, tenant_id, live_session_id, user_id, joined_at, left_at, completion_state FROM live_session_participants WHERE id = $1', [existing.id]);
...
export async function markParticipantComplete(sessionId: string, userId: string): Promise<LiveSessionParticipant | null> {
  ...
  const result = await db.query('SELECT id, tenant_id, live_session_id, user_id, joined_at, left_at, completion_state FROM live_session_participants WHERE id = $1', [participant.id]);
  return result.rows[0] as LiveSessionParticipant;
}
...
export async function getActiveSessionsForAssignment(assignmentId: string): Promise<LiveSession[]> {
  const db = getDb();
  const result = await db.query(`
    SELECT id, tenant_id, assignment_id, teacher_id, state_snapshot_json, status, started_at, ended_at, created_at, updated_at FROM live_sessions
