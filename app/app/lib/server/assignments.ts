...
export async function updateAssignment(id: string, data: UpdateAssignmentData): Promise<Assignment | null> {
  ...
  if (fields.length === 0) {
    const result = await db.query('SELECT id, tenant_id, class_id, teacher_id, title, description, slide_asset_version_id, quiz_asset_version_id, release_at, due_at, status, created_at, updated_at FROM assignments WHERE id = $1', [id]);
    return result.rows[0] as Assignment;
  }
...
export async function releaseAssignment(id: string): Promise<Assignment | null> {
  ...
  const assignmentResult = await db.query('SELECT id, tenant_id, class_id, teacher_id, title, description, slide_asset_version_id, quiz_asset_version_id, release_at, due_at, status, created_at, updated_at FROM assignments WHERE id = $1', [id]);
...
export async function getAssignmentWithRecipients(id: string): Promise<AssignmentWithRecipients | null> {
  ...
  const recipientsResult = await db.query('SELECT id, tenant_id, assignment_id, student_id, visibility_status, assigned_at FROM assignment_recipients WHERE assignment_id = $1', [id]);
...
export async function getAssignmentsForTeacher(teacherId: string, filters?: ListAssignmentsFilters): Promise<Assignment[]> {
  ...
  const query = `
    SELECT id, tenant_id, class_id, teacher_id, title, description, slide_asset_version_id, quiz_asset_version_id, release_at, due_at, status, created_at, updated_at FROM assignments
